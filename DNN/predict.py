...
count = 1
loaded_model = load_model('dnn_model1.sav')
...
for i in result:
    #print(np.argmax(i))
    print(f'{count} {np.argmax(i)} {dict_class[np.argmax(i)]} {name_convert(y[count])}')
...
#print(f"accuracy: {accu(original,result)}")
#print(keras.metrics.sparse_categorical_accuracy(original, result))
#print(accuracy_score(y_true, y_pred))
print(confusion_matrix(original, y_pred))
cm_analysis(original, y_pred, dict_class, ymap=None, title="Confusion Matrix", figsize=(10, 10))
plot_confusion_matrix(original, y_pred, classes=np.asarray(dict_class), normalize=False, title="Confusion Matrix",cmap=plt.cm.Reds)
plt.show()
